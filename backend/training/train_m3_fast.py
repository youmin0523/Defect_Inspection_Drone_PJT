# =============================================
# train_m3_fast.py
# 역할: M3 빠른 다단계 재학습 (5/4 04시 전 완료 목표)
...
PROJECT = "runs"
NAME_S1 = "m3_fast/stage1"
NAME_S2 = "m3_fast/stage2"
WEIGHTS_DIR = Path("../models_weights")
ONNX_NAME = "m3_yolo_floor_window.onnx"
...
def main():
    start = time.time()
    print("=" * 60)
    print("M3 fast 다단계 재학습")
    print("Stage 1: yolov11l fresh + 30ep + batch=16 + imgsz=640")
    print("=" * 60)

    # Stage 1 (옵션 A: 04시 데드라인 안에 끝나도록 빠른 설정)
    ...
    # Stage 1 best 위치
    s1_best_options = [
        Path(PROJECT) / "stage1" / "weights" / "best.pt",
        Path("runs/detect") / NAME_S1 / "weights" / "best.pt",
        Path(PROJECT) / NAME_S1 / "weights" / "best.pt",
    ]
    s1_best = None
    for p in s1_best_options:
        if p.exists():
            s1_best = p
            break
    if s1_best is None:
        # Find recursive
        for p in Path(".").rglob("m3_fast/stage1/weights/best.pt"):
            s1_best = p
            break
    ...
    s2_best_options = [
        Path("runs/detect") / NAME_S2 / "weights" / "best.pt",
        Path(PROJECT) / NAME_S2 / "weights" / "best.pt",
    ]
    s2_best = None
    for p in s2_best_options:
        if p.exists():
            s2_best = p
            break
    if s2_best is None:
        for p in Path(".").rglob("m3_fast/stage2/weights/best.pt"):
            s2_best = p
            break

    # Stage 1 vs 2 mAP 비교 후 더 좋은 것 채택
